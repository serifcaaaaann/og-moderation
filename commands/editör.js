...
class Streamer extends Command {
    ...
    async run(message, args, perm) {
        ...
        if(!user.roles.cache.has(ayarlar.editör)) {
            await this.client.yolla(`${user} kişisine <@&${ayarlar.editör}> rolü verildi.`, message.author, message.channel)
            user.roles.add(ayarlar.editör)
        } else{
            await this.client.yolla(`${user} kişisinden <@&${ayarlar.editör}> rolü alındı.`, message.author, message.channel)
            user.roles.remove(ayarlar.editör)
        }
    }
}

module.exports = editör;