...
class Rolyonet extends Command {
    ...
    async run(message, args, level) {
        let yetkiRolu = [ayarlar.yetkirolleri]
        ...
        if (!args[0]) return this.client.yolla("Kullanımı: .r al/ver Kullanıcı Rol", message.author, message.channel)
        if (args[0] != "al") {
            if (args[0] != "ver") {
                return this.client.yolla("Kullanımı: .r al/ver Kullanıcı Rol", message.author, message.channel)
            }
        }
        ...
        if (yetkiRolu.includes(role.id) && !message.member.roles.cache.has(ayarlar.kurucu) && !message.member.roles.cache.has(ayarlar.ustyonetici)) return this.client.yolla("Yetki rolleri ile işlem yapamazsın.", message.author, message.channel)
        ...
        if (args[0] == "al") {
            ...
            if (user.roles.cache.has(role.id)) {
                ...
                const emeed = new Discord.MessageEmbed()
                    ...
                    .setTimestamp()
                this.client.channels.cache.get(ayarlar.rollog).send(emeed)
                roller.findOne({
                    user: user.id
                }, async (err, res) => {
                    if (!res) {
