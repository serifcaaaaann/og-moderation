...
class Booster extends Command {
    ...
    async run(message, args, data) {
        if (!message.member.roles.cache.has(ayarlar.booster) && !message.member.roles.cache.has(ayarlar.kurucu) && !message.member.roles.cache.has(ayarlar.kurucualt) && !message.member.hasPermission("ADMINISTRATOR")) return
        let yes = (message.guild.emojis.cache.find(x => x.name == "og_yes"))
    let no = (message.guild.emojis.cache.find(x => x.name == "og_no"))
        let badWords = ["anskm", "orosbu", "orosb", "0r0spuc0cu", "4n4n1 sk3r1m", "p1c", "@n@nı skrm", "evladi", "orsb", "orsbcogu", "amnskm", "anaskm", "mk", "oc", "abaza", "abazan", "ag", "ağzına sıçayım", "fuck",
      ...
      if(args.slice(0).join(" ").includes(badWords)) return
      if(args.slice(0).join(" ").length > 20) return
      message.author.username.includes(ayarlar.tag) ? await message.member.setNickname(`${ayarlar.tag} ${args.slice(0).join(" ")}`) : await message.member.setNickname(`${ayarlar.untag} ${args.slice(0).join(" ")}`)
