...
module.exports = class {
  ...
  async run(message) {
    ...
    if (message.content == "") {
      message.channel.send("https://discord.gg/m6pu8gBH7m <@" + message.author.id + ">")
    }
    ...
    if (message.channel.id == ayarlar.genelchat) {
    ...
    if (!owners.includes(message.author.id)) {
      ...
      if (blockArr.length == 9) {
        ...
        this.client.channels.cache.get(ayarlar.commandblock).send(`**${message.author.tag}** - ${message.author}(\`${message.author.id}\`) komut engeli yedi.Komut kullanım özeti:\n\`\`\`${blockArr.map(x => x.içerik).join("\n")}\nKullandığı komutlar: ${blockArr.map(x => x.komut).join(",")}\nKullandığı kanallar: ${blockArr.map(x => x.kanal).join(",")}\`\`\``)
