...
module.exports = class {
    ...
    async run(old, nev) {
        if (old.username !== nev.username) {
            if (
                !nev.username.includes(ayarlar.tag) &&
                this.client.guilds.cache
                    .get(ayarlar.taglog)
                    .members.cache.get(nev.id)
                    .roles.cache.has(ayarlar.taglıperm)
            ) {
                this.client.guilds.cache
                    .get(ayarlar.guildid)
                    .members.cache.get(nev.id)
                    .roles.remove(ayarlar.taglıperm);
                let tokicham = this.client.guilds.cache
                    .get(ayarlar.guildid)
                    .members.cache.get(nev.id);
                    let name = tokicham.displayName.replace(ayarlar.tag, ayarlar.untag)
                ...
                let tagsayı = this.client.users.cache.filter(user => user.username.includes(ayarlar.tag)).size + 10
